...
namespace Plethora.Format
{
    ...
    public class NumericFormatParser<T> : Component, IFormatParserPartial<T>, ICloneable
        where T : IFormattable, IConvertible, IComparable, IComparable<T>
    {
        #region Static Members

        private static readonly NumericFormatParser<T> defaultFormatParser;
        ...
        private static readonly T zeroT;

        /// <summary>
        /// Called the first time the class is accessed.
        /// </summary>
        /// <remarks>
        /// Completing the call to <see cref="GetTryParseDelegate"/> here ensures the 'TryParse'
        /// method of <typeparamref name="T"/> is located on the first calling thread to this method.
        /// This aids debugging if <typeparamref name="T"/> does not contain 'TryParse' as
        /// exception handling off-thread can cause severe failure.
        /// </remarks>
        ...
        public virtual string AssumedSuffix
        {
            ...
            set
            {
                ...
                if (string.Equals(value, assumedSuffix))
                    return;
        ...
        /// <summary>
        /// Returns a value indiating whether the provided suffix should not be explicitly considered when
        /// formatting values.
        /// </summary>
        /// <param name="suffix">
        ...
        /// </returns>
        /// <remarks>
        /// This allows for suffixes which are specifically handled by the .ToString()
        /// method, for example percentages [%].
        /// </remarks>
        protected static bool IsSuffixInExclusionList(string suffix)
        {
            ...
            return
                string.Equals(suffix, numberFormat.PercentSymbol) ||
                string.Equals(suffix, numberFormat.PerMilleSymbol);
        }
        ...
        {
            int result = value.CompareTo(zeroT);

            if (result == 0)
            {
                return this.FormatSubStringZero ?? this.FormatSubStringPositive;
            }
            else if (result == -1)
            {
                return this.FormatSubStringNegative ?? this.FormatSubStringPositive;
            }
            else
            {
        ...
        private string GenerateRegexParseNumericPattern()
        {
            ...
            string regexPatternDigits =
                @"(\d(" + Regex.Escape(numberFormat.NumberGroupSeparator) + @"?\d*)*)?";

            //Test for decimal digits
            string regexPatternDecimal =
                @"(" + Regex.Escape(numberFormat.NumberDecimalSeparator) + @"\d*)?";
        ...
        private static string LiteralToRegexPartial(string s)
        {
            ...
            {
                string c = Regex.Escape(char.ToString(s[i]));
